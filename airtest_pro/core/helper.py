...
from airtest_pro.utils.logwraper import Logwrap, AirtestLogger
from airtest_pro.core.error import NoDeviceError
...
class G(object, metaclass=DeviceMetaProperty):
    ...
    LOGGING = get_logger("airtest_pro.core.api")
    SCREEN = None
    _DEVICE = None
    DEVICE_LIST = []
    RECENT_CAPTURE = None
...
def log(arg, timestamp=None, desc="", snapshot=False):
    ...
    """
    from AirtestPro.core.cv import try_log_screen
    if G.LOGGER:
        depth = 0
        if snapshot:
...
def logwrap(f):
    """
    A decorator used to add the current function to the airtest_pro log,
    ...
    Examples:
        ...
        Add foo() to the airtest_pro report html page::
...
def import_device_cls(platform):
    ...
    if platform in G.CUSTOM_DEVICES:
        ...
        from AirtestPro.core.android.android import Android as cls
    elif platform == "windows":
        from AirtestPro.core.win.win import Windows as cls
    elif platform == "ios":
        from AirtestPro.core.ios import IOS as cls
    elif platform == "linux":
        from AirtestPro.core.linux.linux import Linux as cls
    else:
